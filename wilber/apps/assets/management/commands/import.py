...
def get_user():
    try:
        ...
        u = User.objects.create_user(
            "deviantart", "deviantart@wilber.social", "deviant-art"
        )
    return u
...
@transaction.atomic
def create_asset(
    name, description, category, image_path, asset_file_path, url
):
    u = get_user()
...
def parse_asset(asset, category):
    ...
    try:
        ...
        image = os.path.join(
            asset,
            [f for f in files if f.rsplit(".", 1)[-1] in ["png", "jpg"]][0],
        )
        asset_file = os.path.join(
            asset, [f for f in files if "zip" in f.rsplit(".", 1)[-1]][0]
        )
