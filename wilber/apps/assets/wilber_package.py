...
class WilberPackage(object):
    ...
    def check_is_wilber_package(self, file_list, pk, slug):
        ...
        manifest = self.read_manifest(self.zipfile.open(manifest_path), file_list)
    ...
    def check_is_plugin_package(self, file_list, pk, slug):
        ...
        manifest = self.read_manifest(self.zipfile.open(self.manifest_filepath), file_list)
    ...
    def get_path_image(self, image_path, pk, slug):
        ...
        new_image_path = path.join(wilber_path, "%05d_%s%s" % (pk, 'image', ext))
    ...
    def rename_wilber_files(self, manifest, pk, slug):
        ...
        new_manifest_path = path.join(self.temp_dir, self.get_path_manifest(pk, slug))

        image_filepath = path.join(self.temp_dir, manifest['image'])
        new_image_path = path.join(self.temp_dir, self.get_path_image(manifest['image'], pk, slug))
    ...
    def rename_asset_files(self, manifest, pk, slug):
        asset_path = path.join(self.temp_dir, self.package_root, manifest['category'])
    ...
    def process(self, pk, slug):
        ...
        file_list_plugin = sorted([f for f in self.zipfile.namelist() if not f.endswith('/')])
        file_list_wilber = sorted([path.relpath(f, self.package_root) for f in file_list_plugin])
        ...
        if manifest:
            return self.repack(manifest, pk, slug), "Package converted succesfuly"
...
if __name__ == '__main__':
    ...
    package = WilberPackage(path.expanduser("~/Dropbox/projects/wilber/wilber-web/wilber/test/package_file.zip"))
    #package = WilberPackage(path.expanduser("~/Dropbox/projects/wilber/wilber-web/wilber/test/wilber_package.zip"))

    package_path, message = package.process(pk=888, slug="xuxu")
    print(package_path, message)
    #package_path = package.process(pk=888, slug="xuxu")
    import subprocess
